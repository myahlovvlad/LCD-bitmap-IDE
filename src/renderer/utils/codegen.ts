...
export function generateAllScreensCHeader(
  ...
  const tableRows = canvases.map((canvas) => {
    ...
    const byteLength = getScreenByteLength(canvas.width, canvas.height);
    return `  { "${escapeCString(canvas.stateId)}", ${symbolName}, ${byteLength} }`;
  });
...
export type EmbeddedExportFormat =
  ...
  | 'stm8'                 // STM8 (Cosmic/IAR-STM8/SDCC), flash-resident const, no attribute needed
...
export const EMBEDDED_FORMAT_LABELS: Record<EmbeddedExportFormat, string> = {
  ...
  stm8: 'STM8 C header (Cosmic / IAR-STM8 / SDCC)',
  binary: 'Raw binary'
};
...
export function exportScreenEmbedded(
  ...
  switch (format) {
    ...
    case 'rust-embedded': {
      // ImageRaw<BinaryColor> expects row-major, MSB-first data with every row
      // padded to a whole byte. SSD1306 vertical-page bytes are not compatible.
      const bytes = packFrameBufferHorizontalMsb(frameBuffer, width, height);
    ...
    case 'stm8': {
      const bytes = packFrameBuffer(frameBuffer, width, height);
      return generateStm8Header(sym, bytes, width, height, bpr);
    }
    case 'binary': {
      const bytes = packFrameBuffer(frameBuffer, width, height);
      return Uint8Array.from(bytes);
...
/**
 * STM8 compatible header — vertical LSB, plain flash-resident const array.
 * STM8 (Cosmic, IAR-STM8, SDCC) puts `const` data in flash by default, so no
 * storage-class attribute (unlike AVR PROGMEM or ESP-IDF RODATA_ATTR) is needed —
 * just read the array directly, e.g. `uint8_t b = symbol_name[i];`.
 * Compatible: STM8S/STM8L families, Cosmic C, IAR EWSTM8, SDCC --mstm8.
 */
export function generateStm8Header(
  symbolName: string,
  bytes: readonly number[],
  width: number,
  height: number,
  ...
  return [
    ...
    `/* LCD bitmap — 1bpp, vertical pages, LSB at top (SSD1306 native)   */`,
    `/* Dimensions: ${width}x${height} px  |  ${bytes.length} bytes              */`,
    `/* const data is flash-resident by default on STM8 (Cosmic/IAR/SDCC) */`,
    `/* Generated by ${PRODUCT_IDENTITY.name ?? 'LCD-bitmap-IDE'}                */`,
    '',
    `const uint8_t ${sym}[${bytes.length}] = {`,
    rows.join(',\n'),
    '};',
    '',
    `#define ${sym.toUpperCase()}_WIDTH   ${width}`,
