...
- **UX validation:** a typed UX semantic contract (screen/state/transition/control roles, intents, terminology, user goals, policies) plus a deterministic rule engine (`analyze_project_ux`) that flags dead-end states, missing recovery paths, inconsistent terminology and unconfirmed destructive actions — with an optional, non-blocking external-LLM review channel. See [docs/UX_SEMANTIC_VALIDATION.md](docs/UX_SEMANTIC_VALIDATION.md).
